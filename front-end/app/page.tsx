...
export default function Home() {
  ...
  const {teams, roster, setRoster, starters, setStarters, playerDictionary, setPlayerDictionary} = useAppContext()
  ...
  const generatePlayers = (position: string, amount: number) => {
    ...
    for(var i = 0; i < amount; i++){
      ...
      generatedPlayers.set(name, new Player(position, contract, attributes))
      defaultPlayerDictionary.set(name, new Player(position, contract, attributes))
    }
    const entriesArray = Array.from(generatedPlayers.entries())
    const sortedEntries = entriesArray.sort((a, b) => a[1].attributes.overall - b[1].attributes.overall)
  ...
  const defaultStarters: Map<string, Map<string, Player>> = new Map([
    ["QB", new Map([...defaultRoster.get("QB")!].slice(0, 1))],
    ["WR", new Map([...defaultRoster.get("WR")!].slice(0, 2))],
    ["RB", new Map([...defaultRoster.get("RB")!].slice(0, 1))],
    ["TE", new Map([...defaultRoster.get("TE")!].slice(0, 1))],
    ["OL", new Map([...defaultRoster.get("OL")!].slice(0, 6))],
    ["DL", new Map([...defaultRoster.get("DL")!].slice(0, 4))],
    ["LB", new Map([...defaultRoster.get("LB")!].slice(0, 3))],
    ["CB", new Map([...defaultRoster.get("CB")!].slice(0, 2))],
    ["S", new Map([...defaultRoster.get("S")!].slice(0, 2))],
    ["K", new Map([...defaultRoster.get("K")!].slice(0, 1))],
    ["P", new Map([...defaultRoster.get("P")!].slice(0, 1))]
  ])
  ...
  const handleSaveSelect = async (name: string) => {
    ...
    const processPlayer = (player: any) => {
      ...
      const tempPlayer = new Player(playerPosition, contract, attributes)
